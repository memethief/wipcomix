...
function drawStripNumber($strip_number=null) {
	...
	//die("foo");
	if ($strip_number == null) $strip_number = getStripNumber();
...
function get_ComixFactory() {
	...
	switch ($source) {
	case 'xml': return new ComixXML();
	case 'db': return new ComixDB();
	default: throw new Exception("Unknown comic source passed: [$source]");
	}
}
...
function draw_head($file, $cx, $cy, $facing=0, $scalex=1, $scaley=1, $scalez=1) {
	$default = Array ( 
		'facewd' => 1.0, 'faceht' => 2.0, 'facedp' => 1.0,
		'skulwd' => 1.2, 'skulht' => 1.5, 'skuldp' => 1.8,
		);
	$facings = Array (
		 0 => Array( 'theta' => 0, 'phi' => 0 ),
		 1 => Array( 'theta' => pi()/4, 'phi' => 0 ),
		 2 => Array( 'theta' => pi()/2, 'phi' => 0 ),
		 3 => Array( 'theta' => 3*pi()/4, 'phi' => 0 ),
		 4 => Array( 'theta' => pi(), 'phi' => 0 ),
		-3 => Array( 'theta' => 5*pi()/4, 'phi' => 0 ),
		-2 => Array( 'theta' => 3*pi()/2, 'phi' => 0 ),
		-1 => Array( 'theta' => 7*pi()/4, 'phi' => 0 ),
		);
